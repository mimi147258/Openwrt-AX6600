...
| 项 | 推荐值 | 说明 |
|----|--------|------|
| 地区 | `US` | 可用信道多、功率上限高，全部推荐信道均为非 DFS，避免雷达检测断流 |
| 2.4G | 信道 `11`，带宽 `20MHz` | 抗干扰优先 |
| 5G | 信道 `149`，带宽 `80MHz`（环境干净可试 160MHz） | 雅典娜为 IPQ6018 系平台，5G 建议用高频段信道；`149` 起在 US 下功率上限最高 |
| 发射功率 | `24 dBm` | US 法规内的高功率档 |
| MIMO | 开启 | 多天线并发，提升多设备吞吐 |
| 加密 | `WPA2-PSK` + `CCMP` | 兼容性最稳组合，老设备无障碍接入 |
